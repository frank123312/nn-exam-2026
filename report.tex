...
\IfFileExists{generated/report_values.tex}{\input{generated/report_values.tex}}{}
...
从表中可以看到，SGD 与对角经验 Fisher 预条件方法均已训练并进行了比较。在当前超参数下，SGD 在五个随机种子上更稳定，第三轮平均测试准确率为 $\QSixSGDEpochThreeMean\pm\QSixSGDEpochThreeStd$；对角经验 Fisher 方法第三轮平均测试准确率为 $\QSixNGEpochThreeMean\pm\QSixNGEpochThreeStd$，标准差较大，说明该近似自然梯度方法对随机初始化和超参数更加敏感。因此，本实验结论不是“近似自然梯度一定优于 SGD”，而是：在该实现和参数设置下，SGD 更稳定；对角经验 Fisher 方法在部分随机种子下可以取得较高准确率，但整体波动更大。
...
题目中提到“最好给出交互式界面”。本文主报告展示的是固定终点的可复现实验图；项目中另外提供了交互式脚本 \texttt{q7\_maze\_interactive.py}。运行该脚本后，会弹出迷宫窗口，用户可用鼠标点击任意可达格点作为终点，程序随后训练 Q-learning 策略、输出 BFS 最短路径长度与 Q-learning 路径长度，并在窗口中画出路线。也可以使用命令行方式指定终点，例如：
\begin{verbatim}
python3 q7_maze_interactive.py --goal 8 45
\end{verbatim}
该交互界面作为附加实现，不参与 \texttt{run\_all.py} 的自动批量运行，以避免批处理时阻塞等待鼠标点击。
